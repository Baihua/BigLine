...
Intersection BVHAccel::getIntersection(BVHBuildNode* node, const Ray& ray) const
{
	...
	// TODO Traverse the BVH to find intersection
	Vector3f invDir;
	invDir.x = ray.direction.x == 0.0f ? std::numeric_limits<float>::max() : 1.f / ray.direction.x;
	invDir.y = ray.direction.y == 0.0f ? std::numeric_limits<float>::max() : 1.f / ray.direction.y;
	invDir.z = ray.direction.z == 0.0f ? std::numeric_limits<float>::max() : 1.f / ray.direction.z;
	...
	if (node->bounds.IntersectP(ray, invDir, dirIsNeg)) {
		...
		if (left.happened && right.happened)
		{
			...
			left.normal = normalize((left.normal + right.normal));
			return left;
		}
		return left;
