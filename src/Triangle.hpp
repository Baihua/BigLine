...
public:
	MeshTriangle(const std::vector<int>& indices, const std::vector<float>points, Material* mt = new Material()) {
		...
		for (int i = 0; i < indices.size(); i += 3) {
			...
			for (int j = 0; j < 3; j++) {
				auto vert = Vector3f(points[3*indices[i + j]], points[3 * indices[i + j]+1], points[3 * indices[i + j]+2]);
...
{
	Intersection inter;
//	int dir = dotProduct(ray.direction, normal) > 0 ? -1: 1;
	...
	if (t_tmp < ray.t_min + EPSILON)
		return inter;
	if (t_tmp + EPSILON > ray.t_max)
		return inter;
	//if (t_tmp - ray.t_min < EPSILON)
